...
```bash
# 1. 把项目复制到服务器
scp -r pinganyunwei root@your-server:/opt/sosreport-rca
...
```ini
[Unit]
Description=SOSReport RCA Web Service
After=network.target

[Service]
Type=simple
User=root
WorkingDirectory=/opt/sosreport-rca
Environment=DEEPSEEK_API_KEY=sk-your-key-here
Environment=DEEPSEEK_BASE_URL=https://api.deepseek.com
ExecStart=/usr/bin/python3 -c "import sys; sys.path.insert(0, '/opt/sosreport-rca'); from src.web import app; app.run(host='0.0.0.0', port=8080, debug=False)"
Restart=always
RestartSec=5
...
systemctl enable sosreport-rca
systemctl start sosreport-rca
```
